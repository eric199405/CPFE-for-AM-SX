...
registerMooseObject("am_sx_superalloyApp", PiecewiseFunctions);

InputParameters
PiecewiseFunctions::validParams()
{
...
PiecewiseFunctions::PiecewiseFunctions(const InputParameters & parameters)
  : Function(parameters),
...
unsigned int
PiecewiseFunctions::getFunctionIndex(Real t, const Point & p) const
{
...
Real
PiecewiseFunctions::value(Real t, const Point & p) const
{
...
RealVectorValue
PiecewiseFunctions::gradient(Real t, const Point & p) const
{
  const unsigned int i = getFunctionIndex(t, p);
...
Real
PiecewiseFunctions::timeDerivative(Real t, const Point & p) const
{
  const unsigned int i = getFunctionIndex(t, p);
