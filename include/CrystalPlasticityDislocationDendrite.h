...
protected:
  ...
  // relative intensities of TGMs
  const Real _k_solute; // type 4
  const Real _k_HL; // type 2
  const Real _k_w; // type 3

  // enable (1) and disable (0) TGMs
  const int _inhomo_shear; // type 2
  const int _inhomo_or; // type 3
  const int _inhomo_solute; // type 4
